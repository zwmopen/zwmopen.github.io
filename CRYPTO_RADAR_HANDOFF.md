...
跨平台回归：在 390px 宽度进入 KuCoin / Bybit / Gate，确认 KuCoin 显示 `12`、`头像 3/12`、`主页 0/12`；Bybit / Gate 不显示种子用户，显示空榜原因和官方榜链接；顶部与正文均无横向溢出。

## 已知未完成事项

1. 尚未发现可匿名、可持续且能证明为 Personal Trader 的官方排行榜接口。
2. 尚未把 Personal Trader 详情页接到独立公开 profile URL；必须等稳定源提供 profile/id。
3. Bybit 与 Gate 仍需要稳定、可重复的官方采集链路；在此之前保持空榜和官方链接，不接入无法统一核验的第三方用户。
4. 主题切换改动尚在本轮分支，尚未创建或合并对应 PR；此前公共仓库核心 PR #16/#17/#18 与私有仓库 PR #4 已合并并验证过 Pages。
