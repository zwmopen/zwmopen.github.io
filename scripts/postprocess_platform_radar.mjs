...
function idFor(user){return String(user?.rawId||user?.wallet||user?.name||'unknown').trim().toLowerCase().replace(/[^a-z0-9\u4e00-\u9fff._:-]+/g,'_').slice(0,120)}
function cleanMetrics(metrics={}){const out={};for(const [k,v] of Object.entries(metrics)){const x=Number(v);if(Number.isFinite(x))out[k]=Math.round(x*1e8)/1e8}return out}
function ingest(history,snapshot,at){
  if(!at||!Number.isFinite(Date.parse(at)))return;
  history.series=history.series||{};
  for(const [platform,modes] of Object.entries(snapshot.platforms||{}))for(const [mode,board] of Object.entries(modes||{})){
    if(!Array.isArray(board?.users))continue;
    for(const user of board.users){
      const metrics=cleanMetrics(user.metrics);if(!Object.keys(metrics).length)continue;
      ...
      const point={at,metrics},raw=bucket.raw||(bucket.raw=[]),lastRaw=raw.at(-1);
...
const history=data.history||{updatedAt:null,series:{}};
...
for(const key of ['mexc','htx']){
  ...
  for(const user of board.users||[])for(const field of ['roi','mdd','winRate']){
    const value=Number(user.metrics?.[field]);if(Number.isFinite(value))user.metrics[field]=Math.round(value*100*1e6)/1e6;
  }
  board.percentNormalized=true;board.metricUnit='percentage-normalized';
}
...
async function binanceWeb3(){
  ...
  const res=await fetch(endpoint,{headers:{'Accept-Encoding':'identity','User-Agent':'binance-web3/3.0 (Crypto Radar)'}}),body=await res.json();
  ...
  return (body?.data?.data||[]).map(item=>{
    const address=String(item.address||'').trim(),name=String(item.addressLabel||item.label||(address?`${address.slice(0,6)}…${address.slice(-4)}`:'')).trim();
...
try{
  const users=await binanceWeb3();data.diagnostics=data.diagnostics||{};data.diagnostics['binance:trader']={at:now,found:users.length,error:null,source:'official-web3-public-endpoint'};
  if(users.length>=3){data.platforms.binance=data.platforms.binance||{};data.platforms.binance.trader={title:'Binance Web3 链上交易员榜',period:'30D',sourceUrl:'https://www.binance.com/zh-TC/skills/detail/binance-web3/crypto-market-rank',generatedAt:now,users}}
}catch(error){data.diagnostics=data.diagnostics||{};data.diagnostics['binance:trader']={at:now,found:0,error:String(error)}}

data.generatedAt=now;ingest(history,data,now);data.history=history;
